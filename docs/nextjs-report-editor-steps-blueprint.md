...
本蓝图基于当前后端能力与管理端目标，给出一套可直接落地的 Steps 向导式编辑方案（当前保存即生效）。
...
- 至少包含一个 chapter，且每个 chapter 至少有一个 section
...
- savedAt: 最近一次保存时间（可选）
- payloadHash: 最近一次保存返回的 payload 哈希（可选）
